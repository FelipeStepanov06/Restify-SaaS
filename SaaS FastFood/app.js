...
async function fetchBackendData() {
  try {
    ...
    const resIng = await fetch(`${API_BASE_URL}/ingredients`);
    if (resIng.ok) {
      ingredients = await resIng.json();
    }
    console.log('Dados carregados do backend com sucesso!');
  } catch (e) {
    console.warn('Backend indisponível. Usando fallback de dados locais.', e);
    // Caso backend offline, tenta puxar do painel admin (localStorage)
    const localProds = localStorage.getItem('r_products');
    if (localProds) {
...
function openCustomizationScreen(product) {
  ...
  // Renderiza Ingredientes Baseados na Categoria do Produto
  const ingredientsSection = $('ingredients-section');
  ingredientsSection.innerHTML = '';
  
  const validIngredients = ingredients.filter(ing => {
    return !ing.cats || ing.cats.includes(product.cat);
  });
  ...
  if (validIngredients.length > 0) {
    ...
    validIngredients.forEach(ingredient => {
      ...
      card.innerHTML = `
        ...
        <span style="font-size:11px;color:var(--primary)">${priceLabel}</span>
      `;
  ...
  // Renderiza Extras (Bebidas, Acompanhamentos, Sobremesas) do Lado Direito
...
function confirmCustomizationAndAdd() {
  ...
  const extrasTotal = selectedIngredients.reduce((sum, item) => sum + item.price, 0);
  const modifications = selectedIngredients.map(item => item.name);
  
  // Adiciona Lanche Principal (Order Item Model)
  cart.push({
  ...
  // Adiciona Itens Extras selecionados na direita
...
async function submitOrderToBackend() {
  ...
  try {
    ...
    console.warn('Backend indisponível. Salvando pedido localmente.', error);
    
    orderId = orderCounter.toString().padStart(3, '0');
    orderCounter++;
    localStorage.setItem('r_oc', orderCounter.toString());
    
    const history = JSON.parse(localStorage.getItem('r_orders') || '[]');
    history.push({ id: orderId, ...orderData });
    localStorage.setItem('r_orders', JSON.stringify(history));
  }
...
function initializeApplication() {
  ...
  // Sincronização em Tempo Real com o Painel Admin (via localStorage)
  window.addEventListener('storage', (event) => {
    if (event.key === 'r_products') {
      try {
        ...
        console.error('Erro ao sincronizar produtos do admin.', e);
      }
    }
  });
}
